 ...
 * Class ErrorHandler
 */
class ErrorHandler extends CApplicationComponent
{
  /**
  ...
  /**
   * The exceptions remote endpoint
   *
   * @var string
   */
  public $endpoint;
  ...
  /**
   * A label for the environment e.g. [FSW]
   * @var string
   */
  public $environment;
  ...
  public function getClient()
  {
    if ($this->_client === null) {
      ...
      $config = new Airbrake\Configuration($this->apiKey, $options);
      $this->_client = new Airbrake\Client($config);
    }
  ...
  /**
   * Throw an exception out to Codebase using the AirBrake event handler
   *
   * @param Exception $exception The exception object
   *
   */
  private function airbrakeException($exception)
  {
    $this->getClient()->notifyOnException($exception);
  }

  /**
   * Throw PHP errors out to Codebase
   *
   * @param $error
   */
  private function airbrakeError($error)
  {
    $message = '['.$error['type'].'] '.$error['message'].PHP_EOL;
    $message .= $error['file'].' ('.$error['line'].')';
    $this->getClient()->notifyOnError($message);
  }
  ...
  protected function handleException($exception)
  {
   ...
   $accepted_error_codes = array(400, 404);
   $code = ($exception instanceof CHttpException)?$exception->statusCode:500;

   if (!in_array($code, $accepted_error_codes)) {
     $this->airbrakeException($exception);
   }
